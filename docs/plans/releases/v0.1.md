...
| R2 | in_progress | condition and bounded loop | Tasks 1-2 approved through `cfdf43a` |
...
- R2 Tasks 1-2 are complete and independently approved through commit `cfdf43a`.
...
- Next required action: R2 Task 3 Step 1, creating
  `tests/unit/workflow/test_program.py`.
- After that file exists, the first RED command is
  `.\.venv\Scripts\python.exe -m pytest tests/unit/workflow/test_program.py -q`.
- R2 remains in progress; Tasks 3-5 have not started.
