...
- `closing`：拒绝新 Run，等待既有 Run 到达终态。
- `closed`：不允许新执行，历史、事件和分析仍可查询。
- `deleting`：拒绝所有访问，执行清理事务和 Artifact 删除。

`deleted` 不是持久状态：删除完成后，Session 及其 SDK 数据不存在，按 id 查询返回 not found。

删除不会撤销工具对用户 workspace 或外部系统造成的副作用。

`close()` 将 active 变为 closing；既有 Run 可完成，最后一个 Run 终止后自动变为 closed。`close(force=True)` 先持久取消既有 Run，再等待其到达终态。`delete()` 默认只接受 closed Session；若仍有非终态 Run、未决 reconciliation 或后台 Artifact 写入则返回 `SessionBusyError`。`delete(force=True, confirm_data_loss=True)` 可取消执行并放弃未决恢复信息，然后清理数据。
...
1. 获取 Run lease；验证 Session 允许该 Run 继续、预算未耗尽且未取消。active 允许启动和继续；closing 只允许关闭前已经存在的 Run 继续，禁止创建新的 Run/Workflow/Child。
...
- `session.created|closing|closed|deleting`
...
- Session close 不启动新 Run；delete 清除事件、投影、评估和分析贡献，删除后查询返回 not found。
