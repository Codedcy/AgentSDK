...
事件写入与提交后分发分离：Runtime 的正确性只依赖事务 Event Store，不依赖外部 Exporter 是否健康。事件不可变性适用于仍被保留的 Session；用户显式删除 Session 是唯一会清除其事件链的生命周期操作。
...
- Session 删除造成的全局 cursor 空洞被正常跳过；删除后不能从本地 Event Store 重放该 Session。
...
Session 删除时，本地事件、投影、Evaluation 和 Analytics contribution 一并清除。提交后向当前在线订阅者发送非持久 `session.deleted` notification；它不重新写入已清空的 Event Store。外部 Exporter 的数据保留由应用控制。
