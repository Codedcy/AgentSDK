...
`persistent` 规则属于 SDK/Application Policy Store，而非创建它的 Session，因此 Session 删除后仍保留，直到用户或应用撤销。为满足 Session 数据删除，规则只保留规范化资源、决定和全局 actor，不保留原 Session id、对话文本或 ToolCall payload；来源审计在删除时改为 `source_session_deleted`。
